...
export const RouteDemo: React.FC = () => {
  ...
  // Handle plan toggle with reset animation
  ...
  return (
    <section id="interactive-demo" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#090A0F] border-t border-white/5 relative">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          {/* Plan Toggle Control */}
          <div className="flex items-center p-1.5 rounded-xl bg-surface border border-white/10 space-x-2">
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-8 bg-[#0E1017] border border-white/10 rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between">
            {/* Top Bar Status */}
            <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-4 z-10">
              ...
              <button
                ...
                className="px-3 py-1 rounded-lg bg-white/5 hover:bg-white/10 text-xs font-mono text-slate-300 border border-white/10 flex items-center space-x-1.5"
              >
            ...
            <div className="relative w-full h-[400px] rounded-xl bg-[#08090D] border border-white/5 overflow-hidden">
              <svg viewBox="0 0 800 550" className="w-full h-full object-contain">
                ...
                {activePlan.routeSequence.map((locId, idx) => {
                  ...
                  return (
                    <motion.line
                      ...
                      x2={currLoc.x}
                      y2={currLoc.y}
                      stroke={activeTab === "current" ? "#F59E0B" : "#10B981"}
                      strokeWidth="3.5"
                      strokeDasharray="6 6"
                ...
                <g transform={`translate(${HUB_LOCATION.x}, ${HUB_LOCATION.y})`}>
                  <circle r="16" fill="#4F46E5" stroke="#818CF8" strokeWidth="2.5" />
                  <text y="-22" textAnchor="middle" fill="#FFFFFF" fontSize="15" fontWeight="bold">
                    Birmingham (Hub)
                  </text>
                </g>
                ...
                {DESTINATIONS.map((dest) => {
                  ...
                  const isIndependent =
                    activeTab === "optimised" && isLeicester;

                  return (
                    <g key={dest.id} transform={`translate(${dest.x}, ${dest.y})`}>
                      <circle
                        r="10"
                        fill={isIndependent ? "#0369A1" : "#1E293B"}
                        stroke={isIndependent ? "#38BDF8" : "#94A3B8"}
                        strokeWidth="2"
                      />
                      <text
                        y="-16"
                        textAnchor="middle"
                        fill={isIndependent ? "#38BDF8" : "#E2E8F0"}
                        fontSize="14"
                        fontWeight="600"
                      >
                        {dest.name}
                      </text>
                      <text y="24" textAnchor="middle" fill="#94A3B8" fontSize="11" className="font-mono">
              ...
              {activeTab === "optimised" && (
                <motion.div
                  ...
                  className="absolute top-16 right-8 bg-sky-950/90 border border-sky-500/40 rounded-xl p-3 shadow-lg flex items-center space-x-3 text-xs"
                >
            ...
            {/* Sub-text CTA toggle */}
            <div className="mt-4 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-400 pt-2 border-t border-white/5 gap-2">
              <span>
                {activeTab === "current"
              ...
              {activeTab === "current" && (
                <button
                  onClick={() => handleTogglePlan("optimised")}
                  className="px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold flex items-center space-x-1.5 transition-all shadow-glow-emerald"
                >
            ...
            <div className="bg-surface border border-white/10 rounded-2xl p-6 space-y-6">
              ...
              <div className="space-y-3 text-xs font-mono">
                <div className="flex justify-between py-1.5 border-b border-white/5">
                  <span className="text-slate-400">Shuttle Vehicle Miles</span>
                  <span className="font-bold text-white">{activePlan.vehicleMiles} mi</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-white/5">
                  <span className="text-slate-400">Paid Passenger Ride Time</span>
                  <span className="font-bold text-white">{activePlan.paidRideTime}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-white/5">
                  <span className="text-slate-400">Vehicle Driver Time</span>
                  <span className="font-bold text-white">{activePlan.vehicleDriverTime}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-white/5">
                  <span className="text-slate-400">Handovers & Waiting</span>
                  <span className="font-bold text-white">{activePlan.handoversWaiting}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-white/5">
                  <span className="text-slate-400">Total Paid Driver Hours</span>
                  <span className="font-bold text-amber-300">{activePlan.totalPaidHours}</span>
                </div>
                <div className="flex justify-between py-1.5">
                  <span className="text-slate-400">Final Car Return Time</span>
                  <span className="font-bold text-emerald-400">{activePlan.lastCarHome}</span>
                </div>
              </div>
            </div>
            ...
            <AnimatePresence>
              {activeTab === "optimised" && (
                <motion.div
                  ...
                  className="p-6 rounded-2xl bg-gradient-to-br from-emerald-950/60 via-surface to-slate-900 border border-emerald-500/40 shadow-glow-emerald space-y-4"
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs font-mono pt-2 border-t border-emerald-500/20">
                    ...
                    <div className="text-emerald-200 col-span-2">
