...
import { Bot, SlidersHorizontal, ArrowRight } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="max-w-3xl space-y-4">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/20 text-xs font-mono text-sky-700 dark:text-sky-400 font-bold">
            <span>SECTION 09 • THE PRODUCT VISION</span>
          </div>
          ...
          <p className="text-base sm:text-lg text-slate-600 dark:text-slate-400 leading-relaxed">
            RoutePortal doesn't need to start from scratch. This concept extends the existing foundation into a cost-aware collection engine.
          </p>
        </div>

        {/* Visual Product Thesis Pipeline Comparison */}
        <div className="bg-white dark:bg-surface border border-slate-200/80 dark:border-white/10 rounded-2xl p-6 sm:p-8 space-y-6 shadow-card-light dark:shadow-none">
          <div className="text-xs font-mono font-bold text-indigo-700 dark:text-indigo-400 uppercase tracking-wider">
            PRODUCT EVOLUTION: FROM PROXIMITY TO COST-AWARE PLANNING
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-xs font-mono text-slate-700 dark:text-slate-300">
                <span className="px-2 py-1 rounded bg-slate-200 dark:bg-slate-800">Appointments</span>
                <ArrowRight className="w-3 h-3 text-slate-400" />
                <span className="px-2 py-1 rounded bg-slate-200 dark:bg-slate-800">Proximity</span>
                <ArrowRight className="w-3 h-3 text-slate-400" />
                <span className="px-2 py-1 rounded bg-slate-200 dark:bg-slate-800">Assignment</span>
                <ArrowRight className="w-3 h-3 text-slate-400" />
                <span className="px-2 py-1 rounded bg-slate-200 dark:bg-slate-800">Route</span>
              </div>
            </div>

            {/* CONCEPT EXTENSION FLOW */}
            <div className="p-4 rounded-xl bg-indigo-50/80 dark:bg-indigo-950/30 border border-indigo-300 dark:border-indigo-500/30 space-y-3">
              <div className="text-xs font-mono font-bold text-indigo-800 dark:text-indigo-400 uppercase">
                OPTIMISATION CONCEPT EXTENSION
              </div>
              <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-mono text-indigo-950 dark:text-indigo-200 font-medium">
                <span className="px-2 py-0.5 rounded bg-indigo-200 dark:bg-indigo-900/60">Appointments</span>
                <ArrowRight className="w-3 h-3 text-indigo-500" />
                <span className="px-2 py-0.5 rounded bg-indigo-200 dark:bg-indigo-900/60">Constraints + Costs</span>
                <ArrowRight className="w-3 h-3 text-indigo-500" />
                <span className="px-2 py-0.5 rounded bg-indigo-200 dark:bg-indigo-900/60">Optimisation</span>
                <ArrowRight className="w-3 h-3 text-indigo-500" />
                <span className="px-2 py-0.5 rounded bg-indigo-200 dark:bg-indigo-900/60">Driver Mobile App</span>
                <ArrowRight className="w-3 h-3 text-indigo-500" />
                <span className="px-2 py-0.5 rounded bg-emerald-200 dark:bg-emerald-900/60 text-emerald-950 dark:text-emerald-200 font-bold">Actual Cost Data</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* AI vs Deterministic Table */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-200 dark:border-white/10">
          <div className="p-5 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 space-y-3 shadow-sm dark:shadow-none">
